...
class EncoderDecoder(Chain):
    ...
    def __init__(self, vsize_enc, vsize_dec,
        ...
        if attn > 0:
            ...
            self.add_link("attent", L.Linear(4*n_units, 2*n_units))#!!!!!!!!this method is to mix the attention and the previous hidden state matrix up.
    ...
    def feed_lstm(self, word, embed_layer, lstm_layer_list, train):
        ...
        embed_id = F.dropout(embed_id, self.dropratio)
       # # feed into first LSTM layer
        ...
        for lstm_layer in lstm_layer_list[1:]:
            with chainer.using_config('train', True):
                hs = F.dropout(hs, self.dropratio)
    ...
    def encode_decode_train(self, in_word_list, out_word_list, train=True):
        ...
        for next_word_var in var_dec[1:]:
            ...
            if self.attn == NO_ATTN:
                predicted_out = self.out(F.dropout(self[self.lstm_dec[-1]].h, self.dropratio))
            else:
                # __QUESTION Add attention
                with chainer.using_config('train', True):
                    h_t = self[self.lstm_dec[-1]].h
                    alpha_ts = F.softmax(F.matmul(h_t, enc_states, transb=True))
                    context_t = F.matmul(alpha_ts, enc_states)
                    predicted_out = F.tanh(self.attent(F.dropout(F.concat((context_t, h_t), axis=1),self.dropratio)))
                    predicted_out = self.out(F.dropout(predicted_out,self.dropratio))
        ...
        while pred_count < max_predict_len and (int(pred_word.data) != (EOS_ID)):
            ...
            if self.attn == NO_ATTN:
                ...
                with chainer.using_config('train', True):
                    ...
                    alpha_ts = F.softmax(F.matmul(h_t, enc_states, transb=True))
                    context_t = F.matmul(alpha_ts, enc_states)
                    predicted_out = self.out(F.tanh(self.attent(F.concat((context_t, h_t), axis=1))))
                    alpha_arr = alpha_ts
                    prob = F.softmax(predicted_out)
