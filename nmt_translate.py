...
with open(text_fname['en']) as f:
    print(len(f))
    tokenlist_en=[]
    sent_len_en=[]
    for line in f.readlines():
        #line=line.strip('\n')
        sent_len_en.append(len(line))
        #print (line)
        for words in list(line):
            #re.sub(line,',',' ')
            #re.sub(line,'.',' ')
            #re.sub(line,'!',' ')
            #re.sub(line,'?',' ')
    ...
    print ('number of english token:',len(tokenlist_en))
    print (tokenlist_en)
    #print ('len of enlines',sent_len_en)

with open(text_fname['fr']) as k:
    tokenlist_fr=[]
    sent_len_fr = []
    for line in k.readlines():
        sent_len_fr.append(len(line))
        tokenlist_fr.append(line.split(' '))

    print ('number of japanese token:',len(tokenlist_fr))
    #print ('len of frlines',sent_len_fr)

#=======================
#word_types
with open(text_fname['en']) as f:
    word_type_en = (sorted(set(f)))
    len_of_word_type_en=len(word_type_en)
    print('word type of English:', len_of_word_type_en)

with open(text_fname['fr']) as k:
    word_type_jp = (sorted(set(k)))
...
#==========================
#_UNK_
def len_of_UNK_WORD(word_type):
    unique=[]
    for unique_word in list(word_type):
        if word_type.count(unique_word)==1:
            unique.append(unique_word)

    return len(unique)


a=len_of_UNK_WORD(tokenlist_en)
print('_UNK_ in English',a)
b=len_of_UNK_WORD(tokenlist_fr)
print('_UNK_ in Japanede',b)
...
y= sorted(sent_len_fr)

plt.figure(12)
plt.subplot(221)
plt.hist(x,bins= 500,color='r')
plt.title('English')

plt.subplot(222)
plt.hist(y,bins = 500,color='g')
plt.title('Japanese')
...
def lexical_diversity(text1,text2):
    #print (tokenlist_en)
    print ('diversity of en:',len(text1))
    print ('diversity of fr:',len(text2))
    #print ((set(text1)))
    #print ((set(text2))      )
...
predict(s=0, num=10)
predict(s=10000, num=10, r_filt=.5)
predict(s=10000, num=10, p_filt=.5)
